...
const DiscoverProduct = ({ cartItems, updateCartItems }) => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await fetch(`https://e-commerce-fullstack-backend-0y06.onrender.com/product/get/${id}`);
    ...
    const fetchSuggestProduct = async () => {
      try {
        const response = await fetch(
          `https://e-commerce-fullstack-backend-0y06.onrender.com/product/get/${id}/suggestions`
        );
        const data = await response.json();
  ...
  const addToCart = async () => {
    try {
      const response = await axios.post(
        "https://e-commerce-fullstack-backend-0y06.onrender.com/cart/createCart",
        {
          userId,
